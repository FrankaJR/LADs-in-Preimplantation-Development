...
RESULTOUTFN=sys.argv[7]

ANNOFN = './metadata/FR230814.samplesheet.tsv'
BINARYFN = './data/damid_binary/FR230814.DamID.sample_smooth_binary.{construct}.all_stages.{genotype}.damid_pass.binsize_{binsize:d}.hdf5'
CONSTRUCT = 'Dam-Lmnb1'
...
gt_name = GENOTYPE if GENOTYPE == 'homozygous' else '%s.combined' % GENOTYPE.replace('/','')
fn = BINARYFN.format(construct=CONSTRUCT, genotype=gt_name, binsize=BINSIZE)
